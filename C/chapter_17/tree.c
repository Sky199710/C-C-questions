...
static bool Left(const Item * i1,const Item * i2);
static bool ToRight(const Item * i1, const Item * i2);
static bool Right(const Item *i1,const Item *i2);
static void AddNode (Trnode * new_node, Trnode * root);
static void InOrder(const Trnode * root, void (* pfun)(Item item));
static Pair SeekItem(const Item * pi, const Tree * ptree);
static Pair SeekName(const Item * pi,const Tree * ptree);
...
bool AddItem(const Item * pi, Tree * ptree)
{
	...
	if (SeekName(pi,ptree).child!=NULL)
	{
		Kind * node;
		Kind * temp;
		...
		else
		{
			...
			node->next=NULL;
			for(temp=SeekName(pi,ptree).child->item.many_petkinds;temp->next!=NULL;temp=temp->next)
				continue;
			temp->next=node;
			SeekName(pi,ptree).child->item.count++;
			return true;
		}
	}
...
static Pair SeekName(const Item * pi,const Tree * ptree)
{
    ...
    while (look.child != NULL)
    {
        if (Left(pi, &(look.child->item)))
        {
            look.parent = look.child;
            look.child = look.child->left;
        }
        else if (Right(pi, &(look.child->item)))
        {
            look.parent = look.child;
            look.child = look.child->right;
        }
