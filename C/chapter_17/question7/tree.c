...
bool AddItem(const Item * pi, Tree * ptree)
{
    ...
    {
        SeekItem(pi,ptree).child->item.times++;
		return true;
    }
...
int AppearTimes(const Item * pi,const Tree * ptree)
{
	if(SeekItem(pi,ptree).child==NULL)
		return 0;
	else
		return SeekItem(pi,ptree).child->item.times;
}
